...
export default function Board({
  ...
  const [pressingCell, setPressingCell] = useState<{
    row: number;
    col: number;
  } | null>(null);
  ...
  const handleTouchMove = useCallback(
    (e: React.TouchEvent) => {
      ...
      if (activeId) {
        ...
        if (
          !cell ||
          cell[0] !== pressingCell.row ||
          cell[1] !== pressingCell.col
        ) {
          cancelLongPress();
        }
      }
  ...
  const isDateCell = (label: string) =>
    label === month || label === day || label === dow;
  ...
  return (
    <div
      ...
      style={{
        position: "relative",
        background:
          "linear-gradient(135deg, #e8d5a3 0%, #d4b878 40%, #c9a84c 100%)",
    ...
    >
        ...
        {placements.map((pl) => {
          ...
          return (
            <g
              ...
              style={{
                animation: removingIds.has(pl.pieceId)
                  ? "piece-out 0.3s ease-in forwards"
                  : "piece-in 0.3s cubic-bezier(0.2,0.8,0.4,1)",
              }}
            ...
            >
              ...
              >
                {cells.map(([r, c]) => {
                  ...
                  const x =
                    BOARD_PAD + c * (CELL_PX + GRID_GAP) - (hasL ? ext : 0);
                  const y =
                    BOARD_PAD + r * (CELL_PX + GRID_GAP) - (hasT ? ext : 0);
      ...
      <div
        ...
        style={{ gridTemplateColumns: `repeat(7, ${CELL_PX}px)` }}
      >
                  ...
                  {isHole && (
                    <div
                      ...
                      style={{
                        ...
                        height: 30,
                        background:
                          "radial-gradient(circle at 40% 40%, #f5f5f4 0%, #e7e5e4 40%, #6b4c1a 65%, #3a2408 100%)",
                        boxShadow:
                          "inset 0 2px 5px rgba(0,0,0,0.55), 0 1px 3px rgba(0,0,0,0.4)",
