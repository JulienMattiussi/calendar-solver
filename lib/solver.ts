...
import { PIECES, PIECE_ORIENTATIONS, absoluteCells, type RC, type Rot } from "./pieces";

export interface Placement {
  pieceId: string; row: number; col: number; rot: Rot; flipped: boolean;
}

export function solvePuzzle(month: string, day: string, dow: string): Placement[] | null {
  const orderedTargets: RC[] = [];
  BOARD_ROWS.forEach((row, r) => row.forEach((label, c) => {
  ...
  function bt(remaining: number[]): boolean {
    ...
    for (const cell of orderedTargets) {
      if (!covered.has(`${cell[0]},${cell[1]}`)) { first = cell; break; }
    }
    ...
    for (let pi = 0; pi < remaining.length; pi++) {
      ...
      for (const { cells: oc, rot, flipped } of orientations) {
        for (const [dr, dc] of oc) {
          ...
          if (abs.every(([r, c]) => targetSet.has(`${r},${c}`) && !covered.has(`${r},${c}`))) {
